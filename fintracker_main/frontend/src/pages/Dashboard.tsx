...
import { getUser, getTransactions, createTransaction, deleteTransaction, ParsedTransaction } from '../api';
...
const Dashboard = () => {
  ...
  const handleBulkImport = async (items: ParsedTransaction[]) => {
    if (!authUser) return;
    const created = await Promise.allSettled(
      items.map(t => createTransaction(authUser.user_id, { name: t.name, amount: t.amount, category: t.category }))
    );
    const succeeded = created
      .filter((r): r is PromiseFulfilledResult<Transaction> => r.status === 'fulfilled')
      .map(r => r.value);
    setTransactions(prev => [...succeeded, ...prev].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()));
  };
  ...
  return (
    <div>
      ...
      <div className="dash-layout">
            ...
            <div className="card-header">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                <button
                  ...
                  title="Импорт из фото, голоса или SMS"
                >
                  ⚡ Быстрый ввод
                </button>
        ...
        {/* ─── Правый сайдбар с диаграммой и ИИ ─── */}
      ...
      {showQuickImport && (
        <QuickImport onClose={() => setShowQuickImport(false)} onImport={handleBulkImport} />
      )}
    </div>
  );
};
